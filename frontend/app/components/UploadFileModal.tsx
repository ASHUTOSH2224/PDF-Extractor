...
import { apiService, ExtractorsResponse, ExtractorCategory, ExtractorInfo } from "../services/api";
...
export default function UploadFileModal({ isOpen, onClose, onSubmit, loading = false, projectType }: UploadFileModalProps) {
  const { user } = useAuth();
  ...
  const { data: extractorsData, isLoading: extractorsLoading } = useQuery({
    queryKey: ['extractors'],
    queryFn: () => apiService.getExtractors(),
  });

  // Determine which extractors to show based on project type
  const getRelevantExtractors = (): ExtractorCategory[] => {
    ...
    if (projectType === 'pdf') {
      ...
      return extractorsData.image_extractors;
    }
    
    // If no project type specified, show both (fallback)
    return [...extractorsData.pdf_extractors, ...extractorsData.image_extractors];
  };
  ...
  const handleDrop = useCallback((e: React.DragEvent) => {
    ...
    const validFiles = droppedFiles.filter(file => {
      ...
      // If no project type specified, allow both PDF and images
          ...
          >
            ...
            <p className="text-sm text-gray-500 mb-4">
              {projectType === 'pdf' 
                ...
                : projectType === 'image' 
                ? 'Supports image files (PNG, JPG, GIF, etc.)' 
            ...
            <input
              type="file"
              multiple
              accept={projectType === 'pdf' ? '.pdf' : projectType === 'image' ? '.png,.jpg,.jpeg,.gif,.bmp,.tiff,.webp' : '.pdf,.png,.jpg,.jpeg,.gif,.bmp,.tiff,.webp'}

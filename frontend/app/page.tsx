...
function HomePageContent() {
  ...
  const { data: projects = [], isLoading, error } = useQuery({
    queryKey: ['projects'],
    queryFn: () => apiService.getProjects(token!),
    enabled: !!token,
  });

  // Create project mutation
  const createProjectMutation = useMutation({
    mutationFn: (projectData: any) => apiService.createProject(projectData, token!),
    onSuccess: () => {
  ...
  const deleteProjectMutation = useMutation({
    mutationFn: (projectUuid: string) => apiService.deleteProject(projectUuid, token!),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['projects'] });

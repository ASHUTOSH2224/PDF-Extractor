...
import { apiService, UserRatingBreakdown } from "../../../services/api";
...
export function RatingBreakdownRow({
  projectId,
  documentId,
  jobUuid,
  token,
}: RatingBreakdownRowProps) {
  ...
  useEffect(() => {
    const fetchBreakdown = async () => {
      ...
      try {
        setLoading(true);
        setError(null);
        const data = await apiService.getRatingBreakdown(
          projectId,
          documentId,
          jobUuid,
          token
    ...
    fetchBreakdown();
  }, [projectId, documentId, jobUuid, token]);
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="text-xs">Pages Rated</TableHead>
              ...
              {breakdown.map((item) => (
                <TableRow key={item.user_id || 'unknown'} className="hover:bg-white/50">
                  ...
                  <TableCell className="text-xs text-muted-foreground">
                    {new Date(item.latest_rated_at).toLocaleDateString()}
                  </TableCell>
                </TableRow>
              ))}
            </TableBody>

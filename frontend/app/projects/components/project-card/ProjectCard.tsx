...
import { FileText, FileImage, Clock, User, Trash2 } from "lucide-react";
...
export function ProjectCard({ project, onDelete, deleting }: ProjectCardProps) {
  return (
    <Card className="h-full hover:shadow-md transition-shadow relative group">
      <Link href={`/projects/${project.uuid}`} className="block h-full">
        <CardHeader>
          <div className="flex items-start justify-between mb-2">
            <div className="h-10 w-10 rounded-lg bg-primary/10 flex items-center justify-center">
              {project.file_upload_type === "pdf" ? (
            ...
            <div className="flex items-center gap-3">
              <Badge variant={project.file_upload_type === "pdf" ? "default" : "secondary"}>
                {project.file_upload_type?.toUpperCase() || 'PDF'}
              </Badge>
              <button

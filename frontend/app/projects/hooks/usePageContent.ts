...
import { apiService, DocumentPageContent, DocumentExtractionJob, AnnotationResponse } from '../../services/api';
...
export function usePageContent(
  ...
  const fetchPageContent = useCallback(async (extractorName: string, currentPage: number) => {
    ...
    try {
      ...
      const content = await apiService.getExtractionJobPages(
        projectId,
        documentId,
        selectedJob.uuid,
        token
      ...
      try {
        const ann = await apiService.getAnnotations(documentId, token, { 
